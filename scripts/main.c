...
size_t dump_mac_stats(gNB_MAC_INST *gNB, char *output, size_t strlen, bool reset_rsrp)
{
  ...
  UE_iterator(gNB->UE_info.connected_ue_list, UE) {
    ...
    output = st_append(output,
                       ...
                       ", dlsch_errors %"PRIu64", pucch0_DTX %d, BLER %.5f MCS (%d) %d rbStart %d CCE fail %d\n",
                       stats->dl.errors,
                       stats->pucch0_DTX,
    ...
    output = st_append(output,
                       ...
                       ", ulsch_errors %"PRIu64", ulsch_DTX %d, BLER %.5f MCS (%d) %d (Qm %d deltaMCS %d dB) NPRB %d rbStart %d  SNR %d.%d dB CCE fail %d\n",
                       stats->ul.errors,
                       stats->ulsch_DTX,
