...
void post_process_dlsch(gNB_MAC_INST *nr_mac, post_process_pdsch_t *pdsch, NR_UE_info_t *UE, NR_sched_pdsch_t *sched_pdsch)
{
  ...
  if (harq->round != 0) { /* retransmission */
    ...
    LOG_D(NR_MAC,
          "%d.%2d DL retransmission RNTI %04x HARQ PID %d round %d NDI %d rbStart %d rbSize %d\n",
          frame,
          slot,
          rnti,
          current_harq_pid,
          harq->round,
          harq->ndi,
          harq->sched_pdsch.rbStart,
          harq->sched_pdsch.rbSize);
    ...
    LOG_D(NR_MAC, "Initial HARQ transmission in %d.%d rbStart %d rbSize %d\n",
          frame, slot, sched_pdsch->rbStart, sched_pdsch->rbSize);
    ...
    nr_mac->mac_stats.used_prb_aggregate += sched_pdsch->rbSize;
    UE->mac_stats.rbStart = sched_pdsch->rbStart;
