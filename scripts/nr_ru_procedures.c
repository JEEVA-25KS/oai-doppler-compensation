...
#include "openair2/LAYER2/NR_MAC_gNB/nr_doppler_api.h"
  ...
  if (gNB) {
    ...
    if (fabs(doppler) > 0.1) {
      ...
      apply_pre_doppler_compensation_time(&ru->common.txdata[aa][slot_offset],
                                          ...
                                          sampling_rate);
      LOG_D(PHY,
            "DL Doppler pre-compensation: frame %d slot %d antenna %d UE %04x shift %.2f Hz\n",
            ru->proc.frame_tx,
            slot,
            aa,
            rnti,
            doppler);
      LOG_I(PHY,
            "Doppler compensation done (DL): frame %d slot %d UE %04x shift %.2f Hz\n",
            ru->proc.frame_tx,
            slot,
            rnti,
            doppler);
...
void nr_fep(void* arg)
{
  ...
  if (gNB) {
    ...
    if (fabs(doppler) > 0.1) {
      ...
      apply_ul_doppler_compensation_freq(rxdataF,
                                         fp->ofdm_symbol_size,
                                         startSymbol,
                                         endSymbol,
                                         -doppler,
                                         scs);
      LOG_D(PHY,
            "UL Doppler compensation: frame %d slot %d beam %d UE %04x shift %.2f Hz\n",
            ru->proc.frame_rx,
            slot,
            beam,
            rnti,
            doppler);
      LOG_I(PHY,
            "Doppler compensation done (UL): frame %d slot %d UE %04x shift %.2f Hz\n",
            ru->proc.frame_rx,
            slot,
            rnti,
            doppler);
