...
#include "openair1/PHY/TOOLS/nr_doppler_compensation.h"
#include <math.h>
#include "nr_doppler_api.h"

/* Local (MAC-only) Doppler estimator based on SRS timing */
                                            ...
                                            double current_phase)
{
  ...
  if (time_diff_s > 0.0) {
    ...
    LOG_I(NR_MAC,
          "UE %04x Doppler estimate: raw %.2f Hz, filtered %.2f Hz at %d.%d (phase %.3f rad)\n",
          UE->rnti,
          sched_ctrl->doppler_shift,
          sched_ctrl->doppler_shift_filtered,
          frame,
          slot,
          current_phase);
  }
                                ...
                                nfapi_nr_srs_indication_pdu_t *srs_ind)
{
  ...
  switch (srs_ind->srs_usage) {
    ...
    case NR_SRS_ResourceSet__usage_codebook: {
      ...
      /* Minimal Doppler estimation from average SRS channel phase */
      {
                                       ...
                                       const NR_tda_info_t *tda_info)
{
  ...
  LOG_D(NR_MAC,
        "%4d.%2d Allocate UL retransmission RNTI %04x sched %4d.%2d (rbStart %d, rbSize %d)\n",
        frame,
        slot,
        UE->rnti,
        new_sched.frame,
...
void post_process_ulsch(gNB_MAC_INST *nr_mac, post_process_pusch_t *pusch, NR_UE_info_t *UE, NR_sched_pusch_t *sched_pusch)
{
  ...
  UE->mac_stats.rbStart = sched_pusch->rbStart;
