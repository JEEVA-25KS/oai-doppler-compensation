...
typedef struct {
  ...
  /* UL high-traffic hysteresis */
  int ul_high_cnt;   /* consecutive slots above threshold */
  int ul_low_cnt;    /* consecutive slots below threshold */
  bool ul_high_state;/* true => use rbStart >= 60 */
  /* measured UL rate helper (based on received MAC bytes) */
  uint64_t ul_last_total_bytes;
  frame_t ul_rate_last_frame;
  slot_t ul_rate_last_slot;
...
typedef struct {
  ...
  slot_t slot;
  /// FAPI DL_TTI.request body in which DL allocations are to be made
  nfapi_nr_dl_tti_request_body_t *dl_req;
  /// FAPI TX_DATA.request to carry DL TBs
...
typedef struct gNB_MAC_INST_s {
  ...
  /// whether to print periodic UE stats
  bool print_ue_stats;
  /// maximum number of UEs for periodic stats printing (0 disables)
  int stats_max_ue;
} gNB_MAC_INST;
